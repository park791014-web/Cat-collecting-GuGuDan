(function (global) {
  ...
  function worldIntro(world, after) { var save = v2.storageService.loadSaveData(), progress = ensureProgress(save); if (!world || !world.storyIntro || progress.seenWorldIntros[world.id]) { if (after) after(); return false; } return show(world.storyIntro, { eyebrow: 'WORLD ' + world.order, image: world.artwork && world.artwork.thumbnail, imageAlt: world.title }, function () { mark(function (value) { value.seenWorldIntros[world.id] = true; }); if (after) after(); }); }
  function stageIntro(stage, start) {
    if (!stage || !stage.storyIntro) { if (start) start(); return false; }
    return show(stage.storyIntro, { eyebrow: 'STAGE ' + stage.displayNumber, buttonText: '도전하기' }, function () { mark(function (value) { value.seenStageIntros[stage.id] = true; }); if (start) start(); });
  }
  ...
  function clearStory(stage, after, dismissed, force) { if (!stage || !stage.clearStory) { if (after) after(); return false; } var save = v2.storageService.loadSaveData(), progress = ensureProgress(save); if (progress.seenClearStories[stage.worldId] && !force) { if (after) after(); return false; } return show(stage.clearStory, { eyebrow: '보석 조각 발견', onDismiss: dismissed }, function () { mark(function (value) { value.seenClearStories[stage.worldId] = true; }); if (after) after(); }); }
