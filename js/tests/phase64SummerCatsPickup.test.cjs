...
assert.match(style, /\.summer-season-banner__content\{[^}]*display:flex/);
assert.match(style, /\.summer-season-banner__visual img\{[^}]*object-fit:contain/);
...
assert.equal(banner.children[0].className, 'summer-season-banner__content');
assert.equal(banner.children[0].children[0].textContent, '2026 여름 시즌');
assert.match(banner.children[0].children[1].textContent, /한여름 냥캉스/);
assert.match(banner.children[0].children[1].textContent, /2026\.08\.04~09\.03/);
assert.match(banner.children[0].children[1].textContent, /수집 0\/4/);
assert.equal(banner.children[0].children[2].type, 'button');
assert.equal(banner.children[1].children[0].src, 'assets/cats/seasons/summer_2026/heatwave-flame-cat.jpg');
